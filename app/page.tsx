...
import { Loader2, TrendingUp, Award, Clock, Play, Star, Film, Calendar, AlertCircle } from "lucide-react"
...
export default function HomePage() {
  ...
  if (loading && popularMovies.length === 0 && topRatedMovies.length === 0 && nowPlayingMovies.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-background">
        <div className="text-center space-y-4">
          <div className="relative w-16 h-16 mx-auto">
  ...
  // Show error state if there's an error and no movies loaded
  if (error && popularMovies.length === 0 && topRatedMovies.length === 0 && nowPlayingMovies.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-background">
        <div className="text-center space-y-4 max-w-md mx-auto px-4">
          <AlertCircle className="h-16 w-16 text-primary mx-auto" />
          <h1 className="text-2xl font-bold text-foreground">Unable to Load Movies</h1>
  ...
  // Get featured movie (first popular movie with backdrop)
  const featuredMovie = popularMovies.find((movie) => movie.backdrop_path) || popularMovies[0]
        ...
        {featuredMovie && (
          <section className="relative h-[60vh] md:h-[70vh] overflow-hidden bg-muted/30">
            ...
            <div className="absolute inset-0">
              <Image
                src={tmdbApi.getBackdropUrl(featuredMovie.backdrop_path, "original") || "/placeholder.svg"}
              ...
              />
              {/* Balanced gradient overlays for better readability */}
              ...
              <div className="max-w-2xl">
                ...
                <div className="flex items-center space-x-4 mb-4 animate-fade-in">
                  <div className="flex items-center bg-white/20 backdrop-blur-md rounded-full px-3 py-1 border border-white/30">
                    ...
                    <span className="font-medium text-white text-sm">{featuredMovie.vote_average.toFixed(1)}</span>
                  </div>
                  <div className="flex items-center bg-white/20 backdrop-blur-md rounded-full px-3 py-1 border border-white/30">
                    <Calendar className="h-4 w-4 mr-1 text-white/80" />
                    <span className="text-white text-sm">{new Date(featuredMovie.release_date).getFullYear()}</span>
                  </div>
                </div>
        ...
        <section className="container mx-auto px-4 py-12">
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
              <div>
                <h2 className="text-2xl md:text-3xl font-bold mb-2 text-foreground">Browse Movies</h2>
            ...
            <div className="animate-fade-in">
              ...
              {error && !loading && getCurrentMovies().length === 0 && (
                <div className="text-center py-8">
                  ...
                  <Button onClick={handleRetry} variant="outline" className="btn-professional-outline">
                    Try Again
                  </Button>
                </div>
              )}
